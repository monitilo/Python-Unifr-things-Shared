...
# labels = ["Blank", "D1", "D2","D1+2","D3","D4","D3+4",
#          "M1","M2","M1+2","M3","M4","M3+4"]
...
#26.10.22
# labels = ["Blank", "1L1S ramp Dimer", "1L1S ramp Dimer (again)","1L1S ramp Mono UP","1L1S ramp Mono Down",
#           "1L1S Cycle Dimer","1L1S Cycle Mono UP","1L1S Cycle Mono Down",
#           "Alt ramp Dimer","Alt ramp Dimer (again)", "Alt ramp Mono","Alt Cyc Dimer","Alt Cyc Mono"]
#27.10.22
# labels = ["Blank", "Blank", "Blank","Alt Cyc Dimer","Alt Cyc Mono","Alt ramp Dimer","Alt ramp Mono",
#           "1L1S ramp Dimer","1L1S ramp Mono"]
labels = ["Blank", "Ag10NC origami"]
N_blanks = 1
# %% 
...
# ##bustracting the blanck
# norm_data = data_dict[labels[1]] - data_dict[labels[0]]
# plt.plot(waveleng, norm_data, label=(labels[1]+" Max {}".format(max(data_dict[labels[1]][500:]))))
# plt.legend()
# # plt.xlim(300,800)
# plt.ylim(-0.1,0.25)
# plt.title("All spectras")
...
lista = range(N_blanks,len(labels))
# lista = [1,2,3,4,7,8,9,10]
# lista = [1]
data_savgol = dict()
for i in lista :
...
# data_dict = dict()
fitted_data = dict()
peak_position = np.zeros((len(labels)))* np.nan
for i in lista :
    data_dict[labels[i]] = alldata[i]
    ...
    plt.legend()
    peak_position[i] = waveleng[np.argwhere(ym==max(ym))[-1][-1]+xcut]
    # print("Peaks position", peak_position)

    plt.plot(waveleng, data_dict[labels[i]], label=(labels[i]+" Max {:.2}".format(max(ym))+" peak {:2}".format(peak_position[i])))
plt.legend()
plt.xlim(500,851)
plt.ylim(0,1.5)
plt.title("All spectras")
