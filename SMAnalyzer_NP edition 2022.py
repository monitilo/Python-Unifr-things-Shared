...
#        self.setCentralWidget(self.w)
        ...
        self.btn99_clearall = QtGui.QPushButton('Clear all (ctrl+c)')
...
#        self.label_dir_save.resize(self.label_dir_save.sizeHint())
        ...
        self.btn_save_coordinates = QtGui.QPushButton('SAVE spots coordinates')
        self.btn_save_coordinates.clicked.connect(self.save_coordinates)

        self.btn_load_coordinates = QtGui.QPushButton('LOAD spots coordinates')
...
#        self.options_grid.addWidget(QtGui.QLabel(" "),       8, 0, 1, 3)
        ...
        self.options_grid.addWidget(self.btn_save_coordinates,  16, 0, 1, 1)  # fangjia and Aleks asked for this
...
#'bottom', 'top', 'left', 'right', 'above', or 'below'
    ...
    # Shortcuts
        ...
        QtGui.QShortcut(
            QtGui.QKeySequence('ctrl+c'), self, self.clear_all)
...
#                print("amount of channels in mp4 = ", mp4_channels)
                ...
                self.frames = np.zeros((total_frames,height,width,3), dtype=np.uint8)
#                print(sys.getsizeof(self.frames), "AAAAAAAAAAAAAAAA")
...
#                self.smallroi.sigRegionChanged.connect(self.NP_making_traces)
    ...
    def making_traces(self):
        if not self.JPG:
            ...
            if self.trace_combobox.currentText() == channels[0]:
                a = "Normal"
                # print(a)
            elif self.trace_combobox.currentText() == channels[1]:
                valor = (valor - bgnorm )
            elif self.trace_combobox.currentText() == channels[2]:
                valor = (valor - bgnorm ) / bgnorm
            elif self.trace_combobox.currentText() == channels[3]:
                valor = bgnorm
...
#                print(trace_name)
    ...
    def load_coordinates (self):
        ...
        coordinates_name = filedialog.askopenfilename(filetypes=[("txt", "*.txt")])
