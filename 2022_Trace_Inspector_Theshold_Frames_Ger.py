...
#        self.thresholdSlider.setSingleStep(1)
        ...
        self.label_signal2noise = QtGui.QLabel("Signal2Noise")
        self.label_signal2noise.setFixedWidth(300)
...
#        self.Trace_grid.addWidget(self.btnmaxmin,                   11, 0, 1, 3)
        ...
        self.Trace_grid.addWidget(self.label_signal2noise,          1, 15, 1, 3)
...
#        trace = self.data[:, (int(self.traceSlider.value()))]
        meanON = self.avgmax  # np.mean(trace[np.where(trace > 0.1*new_threshold)])
        self.intensity[int(self.traceSlider.value())] = meanON
        self.signal2noise[int(self.traceSlider.value())] = meanON / np.nanstd(self.data[int(minX):int(maxX), (int(self.traceSlider.value()))])
        self.label_signal2noise.setText("<span style='font-size: 12pt'> <span style='color: balck'>Signal2noise=%0.1f</span>" % self.signal2noise[int(self.traceSlider.value())])

        meanOFF = self.avgmin
        self.weber[int(self.traceSlider.value())] = (meanON - meanOFF) / meanOFF
...
#        print(self.stepintensity)
        self.selection[int(self.traceSlider.value()), 9] = self.step_intensity  # stepintensity
...
"""
        ...
        self.signal2noise_threshold = np.zeros(self.data.shape[1])
        self.weber = np.zeros(self.data.shape[1])
        self.intensity = np.zeros(self.data.shape[1])

        self.graph.clear()
        self.selection = np.zeros((self.data.shape[1], 11))  # +color + Steps columns (5 ==> 9)
        print(self.selection.shape)
...
#        print("2threshold = ", self.selection[int(self.traceSlider.value()),2])
        ...
        meanON_threshold = np.mean(trace[np.where(trace > 0.1*new_threshold)])
        self.signal2noise_threshold[int(self.traceSlider.value())] = meanON_threshold / np.std(trace[np.where(trace > 0.1*new_threshold)])
...
#        self.intensity[int(self.traceSlider.value())] = meanON_threshold
...
#        self.selection[int(self.traceSlider.value()), 3] = minX  # Done in updatelr()
#        self.selection[int(self.traceSlider.value()), 4] = maxX
#        self.selection[int(self.traceSlider.value()), 5] = minX2
#        self.selection[int(self.traceSlider.value()), 6] = maxX2
        self.selection[int(self.traceSlider.value()), 7] = self.signal2noise[int(self.traceSlider.value())]
        self.selection[int(self.traceSlider.value()), 8] = self.intensity[int(self.traceSlider.value())]
        self.selection[int(self.traceSlider.value()), 9] = self.stepintensity
        self.selection[int(self.traceSlider.value()), 10] = self.weber[int(self.traceSlider.value())]
...
#        print(self.selection[int(self.traceSlider.value())-1:int(self.traceSlider.value())+2])
    ...
    def save_badSelection_traces(self):
        ...
        self.selection[int(self.traceSlider.value()), 3] = 0
        self.selection[int(self.traceSlider.value()), 4] = 0
        self.selection[int(self.traceSlider.value()), 5] = 0
        self.selection[int(self.traceSlider.value()), 6] = 0
        self.selection[int(self.traceSlider.value()), 7] = 0
        self.selection[int(self.traceSlider.value()), 8] = 0
        self.selection[int(self.traceSlider.value()), 9] = 0
        self.selection[int(self.traceSlider.value()), 10] = 0
...
#        header = "Traces"+"\t"+"Good(1)_bad(-1)"+"\t"+"Threshold"+"\t"+"Green_minX"+"\t"+"Green_maxX"+"\t"+"Red_minX"+"\t"+"Red_maxX"+"\t"+"Signal2noise"+"\t"+"Intensity Green"+"\t"+"Step (colors)"
        header = "Traces"+"    "+"Good(1)_bad(-1)"+"    "+"Threshold"+"    "+"Green_minX"+"    "+"Green_maxX"+"    "+"Red_minX"+"    "+"Red_maxX"+"    "+"Signal2noise"+"    "+"Intensity Green"+"    "+"Step (colors)"+"    "+"Weber contrast (signal-bkg)/bkg"
        
        # I am not savign anymore the Step by Threshold, neither the signal2noise
